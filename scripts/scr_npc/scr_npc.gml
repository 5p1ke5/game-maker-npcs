...
function NPCStateAttack(_target): NPCState(_target) constructor
{
	static Perform = function(_user)
	{
		//If target is a reference to an object instance that no longer exists, resets target to a Point2.
		//TODO: Make this do something better. Reset to previous state? Just idle around? Wait for further orders?
		//maybe if its a player unit return to following hte player otherwise just hold position
		if (instanceof(target) != "Point2") && (!instance_exists(target))
		{
			target = new Point2(_user.x, _user.y);
		}
		...
		with (_user)
		{
			...
			if (distance_to_point(_target.x, _target.y) > MELEE_RANGE)
			{
				_hDir = sign(_target.x - x);
				_vDir = sign(_target.y - y);
				doll_movement(_hDir, _vDir);
